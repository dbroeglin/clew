...
[example courses](examples/reader-vault/courses/mechanics/course.md).

## Course distribution

Digesting a PDF is expensive; reading the result should not be. A producer runs
the ingestion and authoring pass once, publishes the finished `clew/v1` content
...
side, copying selected courses and the concepts they depend on into a vault and
recording provenance in `.clew/imports.json`.

[`docs/COURSE_DISTRIBUTION.md`](docs/COURSE_DISTRIBUTION.md) is the contract
between them, and
[Proposed ADR-0006](docs/adr/adr-0006-course-distribution-and-import.md) records
the rationale and alternatives. Importing is additive and reversible: it never
writes outside `courses/` and `concepts/`, never touches learner records, and
refuses to overwrite locally modified notes without `--force`.
[`examples/reader-vault`](examples/reader-vault) is a published repository of
this shape, holding two courses that share a concept note.
...
[`clew-import`](.agents/skills/clew-import/SKILL.md) copies published courses
from a git repository or local directory into a learner's vault, merging the
evidence lists of shared concept notes and refusing to overwrite local edits.
It is maintained here rather than generated by APM. It imports content only:
it does not digest PDFs, author sections, or read or write learner records.
See [`docs/COURSE_DISTRIBUTION.md`](docs/COURSE_DISTRIBUTION.md).

### Student learning skills

Four first-party skills build on course-content and learner-model without new
packages, plugins or UI. Ask naturally in chat; Clew Tutor remains responsible
for explicitly activated learning-memory capture.

After adding or editing these files in a running Copilot CLI session, run
`/skills reload`, then `/skills list`. A fresh `copilot skill list` also checks
project discovery; an existing session can retain its previous skill cache.

| Skill | Example request | Result |
| --- | --- | --- |
| [`study-plan`](.agents/skills/study-plan/SKILL.md) | "Make a study plan for this chapter." | A source-linked Markdown checklist in the configured vault's `Learning/Plans/`. |
| [`synthesise`](.agents/skills/synthesise/SKILL.md) | "Make revision cards for this section." | Concise cards in chat; one Markdown note under `artifacts/` only if saving is requested. |
| [`active-recall`](.agents/skills/active-recall/SKILL.md) | "Quiz me on average speed." | One grounded question at a time, followed by feedback after an attempt. |
| [`repair-attempt`](.agents/skills/repair-attempt/SKILL.md) | "Here is my solution; help me fix it." | Feedback on the first demonstrably unsupported step, a hint and an opportunity to retry. |

Plans live at `<configured-external-vault>/Learning/Plans/`, not inside this
clone or the Obsidian application. Edit their normal TODO checkboxes in Obsidian,
or explicitly ask the tutor to update a named task. Practice never automatically
checks a box, and a checked box is not evidence of mastery. Dates are included
only on request; there is no automatic scheduling, reminder or progress
database. The course reader remains read-only and does not display personal
plans or artifacts.

Plan writes are separate from learner-memory capture. They do not create a new
memory format or duplicate the plan into artifacts. The initial workflow is for
non-Git vaults; the existing private-path ignore rules do not cover
`Learning/Plans/`. A Git-managed plan destination requires a separate
privacy/tracking decision before saving. No skill initializes vault Git.

See the [implementation plan](docs/plans/student-learning-skills.md) for
synthetic evaluation scenarios and
[Proposed ADR-0007](docs/adr/adr-0007-student-learning-skills-and-plans.md)
for the work-product/memory boundary. These directories are maintained here;
do not edit APM-generated upstream skills to change their behavior.
...
`scripts/clew_finalize.py` completes an authored course in place, regenerating
navigation links, `source_refs`, contents lists, source hashes and the
verification skeleton from the notes and the source map:

```powershell
uv run python scripts\clew_finalize.py --root path\to\course-repo --course my-course
```

Pass `--check` to report what it would change without writing. The example
vault's synthetic sources are rebuilt reproducibly with
`uv run python examples\make_reader_vault_sources.py`; both helpers use the
`pyyaml` dependency declared in `pyproject.toml`.
